...
if x == 1 then
    for i = 1, 91 do
        -- Format the old and new names
        local oldName = string.format("SCENES.League Game.Draft.Macros.%d) %s", i, old[i])
        local newName = string.format("%d) %s", i, new[i])
        
        -- Rename the macro
        set(oldName, "name", newName)
        --print(string.format("Renamed '%s' to '%s'", oldName, newName))  -- Debugging log
    end
end
